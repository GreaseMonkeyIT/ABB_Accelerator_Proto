...
SCENARIOS = [
    ...
    {"id": "S2", "name": "Large-file I/O starvation", "mechanism": "bulk archive read/write", "triggerable": True},
    ...
    {"id": "S5", "name": "Memory leak + OOM termination", "mechanism": "unbounded growth to the memory limit", "triggerable": True},
]
...
def recommendations():
    ...
    q = {
        "cpu_req": 'sum by(namespace,pod)(kube_pod_container_resource_requests{namespace=~"factory-.*",resource="cpu"})',
        "cpu_lim": 'sum by(namespace,pod)(kube_pod_container_resource_limits{namespace=~"factory-.*",resource="cpu"})',
        "cpu_p95": 'quantile_over_time(0.95, sum by(namespace,pod)(rate(container_cpu_usage_seconds_total{namespace=~"factory-.*",container!=""}[5m]))[1h:5m])',
        "mem_req": 'sum by(namespace,pod)(kube_pod_container_resource_requests{namespace=~"factory-.*",resource="memory"})',
        "mem_lim": 'sum by(namespace,pod)(kube_pod_container_resource_limits{namespace=~"factory-.*",resource="memory"})',
        "mem_p95": 'quantile_over_time(0.95, sum by(namespace,pod)(container_memory_working_set_bytes{namespace=~"factory-.*",container!=""})[1h:5m])',
    }
    ...
    for psi in ("io", "cpu", "memory"):
        q_psi = f'sum by(namespace,pod)(rate(container_pressure_{psi}_stalled_seconds_total{{namespace=~"factory-.*"}}[5m]))'
